...
```text
https://github.com/callahan-cw/todo-app-node-project.git
```
...
- Create a `Jenkinsfile` within the `todo-app-node-project` repo with following pipeline script. (***Teacher should Change the `AWS Account Id` with your own***.)
...
- Connect to Jenkins Server instance with SSH. (***Jenkins Server instance has Docker installed, so we will leverage from it to run the Docker container using the newly created image on AWS ECR***)
...
```bash
docker pull <aws-account-id>.dkr.ecr.us-east-1.amazonaws.com/clarusway-repo/todo-app:latest
```

- Run the `todo-app`.

```bash
docker run -dp --name todo 80:3000 <aws-account-id>.dkr.ecr.us-east-1.amazonaws.com/clarusway-repo/todo-app:latest
```
